...
app.post('/blood' ,async (req,res) =>{
    const { location, timing, date, blood} = req.body;
        const existingCamp = await bloodmodel.findOne({ location, time: timing, date, blood });
       ...
       const usersInLocation = await Usermodel.find({ location: location });
       console.log(usersInLocation+"\n******   end *****\n");
       sendEmailsblood(usersInLocation,newCampp,);//changed
       res.status(200).json({ message: 'Data saved successfully' });
